...
def _apns_create_socket_to_feedback(cert_for):
	return _apns_create_socket((SETTINGS["APNS_FEEDBACK_HOST"], SETTINGS["APNS_FEEDBACK_PORT"]), cert_for)
...
def _apns_send(token, alert, cert_for, badge=None, sound=None, category=None, content_available=False,
	...
	if socket:
		...
		with closing(_apns_create_socket_to_push(cert_for)) as socket:
			socket.write(frame)
...
def apns_send_message(registration_id, alert, cert_for, **kwargs):
	"""
	...
	_apns_send(registration_id, alert, cert_for, **kwargs)


def apns_send_bulk_message(registration_ids, alert, cert_for, **kwargs):
	"""
	...
	with closing(_apns_create_socket_to_push(cert_for)) as socket:
		for identifier, registration_id in enumerate(registration_ids):
			_apns_send(registration_id, alert, cert_for, identifier=identifier, socket=socket, **kwargs)
		_apns_check_errors(socket)
